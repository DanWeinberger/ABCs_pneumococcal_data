...
```{r}
st1 %>%
  filter(Age.Group..years.=='65+' & Year>=2016 & IPD.Serotype !='MISS') %>%
  group_by(Age.Group..years.,IPD.Serotype) %>%
  summarize(N_IPD=sum(Frequency.Count)) %>%
  arrange(-N_IPD) %>%
  mutate(stN=row_number(),
        pcv20 = if_else(IPD.Serotype %in% c('1', '3', '4', '5', '6A', '6B', '7F', '8', '9V', '10A', '11A', '12F', '14', '15B', '18C', '19A', '19F', '22F', '23F','33F'),'PCV20','Other')) %>%
  ungroup()%>%
  ...
  xlab('Serotype order')+
  ggtitle('IPD cases among adults 65+ years of age in the US, 2017-2021')
