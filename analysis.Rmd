...
#install.packages(c("dplyr", "ggplot2", "viridis", "plotly", "scales", "pheatmap", "waffle", "reshape2", "gridExtra"), repos = c("https://cinc.rud.is", "https://cloud.r-project.org"))
library(dplyr)
library(ggplot2)
library(viridis)
library(plotly)
...
s2_pre<- s1 %>%
  ...
  summarize(N_IPD_pre=mean(N_IPD)) %>%
  ungroup() %>%
    tidyr::complete(st,Site,  fill=list(N_IPD_pre=0))  #fills 0
...
trends19f <- s1 %>%
  group_by(Site, year) %>%
  mutate(pct = 100*N_IPD/sum(N_IPD, na.rm=T)) %>%
  filter(st=='19F')

  
p1 <- ggplot(trends19f)+
  geom_line(aes(x=year, y=pct, group=Site, color=Site))+
  theme_classic()
...
ecdc2 %>% 
  filter(st=='19F' & agegrp==5) %>%
  ggplot()+ 
  geom_line(aes(x=year, y=pct, group=country, color=country)) +
  ylim(0,6) +
  theme_classic()+
  facet_wrap(~country)
...
a1 <- readxl::read_excel('./Data/Australia/national-notifiable-diseases-surveillance-system-nndss-public-dataset-pneumococcal-disease-invasive.xlsx', skip=1)  %>%
  rename(agec= 'Age group', st=Serotype, year=Year, indig='Indigenous status', dx='Clinical category') %>%
      ...
      pneumonia = 1*grepl('Pneumonia',dx),
      meningitis = 1*grepl('Meningitis',dx),
...
```{r}
vax <- a1 %>%
  filter(`Vaccination history` %in% c('No vaccine given', 'Vaccine/s given')) %>%
 # group_by(year,agec2 ,st,`Vaccination history`) %>%
  mutate(st19F = if_else(st=='19F',1,0))


vax_adult = vax %>% 
  filter(agec2 %in% c('65+y','40-64y')) %>%
  rename ( pcv13 ="Prevenar 13 (13vPCV)"  ,
           ppsv23 = "Pneumovax 23 (23vPPV)" )

mod1 <- glm(st19F ~as.factor(year) + indig +  agec2 +pneumonia + meningitis +pcv13 +ppsv23   , data=vax_adult, family='binomial')
...
```{r, fig.width=5, fig.height=2}
  ...
  summarize(N_ipd=n()) %>%
      ungroup() %>%
    group_by(st, agec2) %>%
      mutate(IPD_scaled=N_ipd/max(N_ipd, na.rm=T)) %>%
...
```{r, fig.width=5, fig.height=2}
        a1 %>%
  group_by( State,st, year) %>%
  summarize(N_ipd=n()) %>%
      ungroup() %>%
    group_by(st, State) %>%
      mutate(IPD_scaled=N_ipd/max(N_ipd, na.rm=T)) %>%
  filter(st %in% c('19F') & State !='NA' ) %>%
ggplot( aes(x=year, y=IPD_scaled)) +
  geom_line()+
  facet_grid(~State)+
  ylim(0, NA)+
